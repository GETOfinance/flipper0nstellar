// ═══════════════════════════════════════════════════════════════
// Flipper Protocol — AI Reasoning Engine (LLM-Powered)
// Supports multiple LLM providers: OpenAI, OpenRouter, LM Studio, Ollama
...
export class AIReasoningEngine {
  ...
  constructor() {
    ...
    switch (provider) {
      ...
      case "lmstudio":
        this.apiKey = process.env.LMSTUDIO_API_KEY || "lm-studio"; // LM Studio doesn't require real API key
        this.apiUrl = process.env.LMSTUDIO_API_URL || "http://localhost:1234/v1/chat/completions";
        this.model = process.env.AI_MODEL || "local-model";
        break;
        
      case "ollama":
        this.apiKey = "ollama"; // Ollama doesn't require API key
        this.apiUrl = process.env.OLLAMA_API_URL || "http://localhost:11434/v1/chat/completions";
      ...
      default:
        console.warn(`[Flipper AI] Unknown provider '${provider}', defaulting to OpenAI`);
        this.apiKey = process.env.OPENAI_API_KEY || "";
    ...
    this.enabled = this.apiKey.length > 0 || provider === "lmstudio" || provider === "ollama";
    
    if (this.enabled) {
      console.log(`[Flipper AI] LLM engine initialized (provider: ${this.provider}, model: ${this.model})`);
    } else {
      console.log("[Flipper AI] No API key found — using heuristic fallback");
    }
  }
  ...
  async analyzeMarket(market: MarketData, riskSnapshot: RiskSnapshot): Promise<AIAnalysis> {
    ...
    try {
      const systemPrompt = `You are Flipper Protocol's AI risk analysis engine for Stellar DeFi positions.
...
Your analysis directly informs autonomous on-chain protection actions on Stellar testnet.
Keep responses under 200 words. Be direct and technical.`;

      const userPrompt = `Analyze this Stellar market snapshot:

PRICE DATA:
- XLM Price: $${market.price.toFixed(4)}
...
LIQUIDITY:
- Total Liquidity: $${(market.liquidity / 1e6).toFixed(1)}M
...
Respond in this exact JSON format:
{
  ...
  "keyInsights": ["insight1", "insight2", "insight3"]
      ...
      console.warn(`[Flipper AI] LLM analysis failed: ${error.message} — falling back to heuristic`);
      return this.heuristicAnalysis(market, riskSnapshot);
    }
  }
  ...
  async analyzeToken(
    ...
    try {
      const prompt = `Analyze this Stellar token for DeFi risks:
...
Respond in JSON:
{
  ...
  "recommendation": "SAFE|CAUTION|AVOID|CRITICAL_RISK"
      ...
      const response = await this.callLLM([
        { role: "system", content: "You are a DeFi security analyst specializing in Stellar token risk assessment. Be precise and critical." },
      ...
      console.warn(`[Flipper AI] Token analysis failed: ${error.message}`);
      return this.heuristicTokenAnalysis(symbol, address, priceChange, volume, liquidity, holderConcentration);
    }
  }
...
Write a 2-3 sentence executive summary of the threat and recommended action. Be specific with data.`;
  ...
  private async callLLM(messages: GroqMessage[]): Promise<string> {
    ...
    try {
      ...
      console.log(`[Flipper AI] LLM response (${json.usage?.total_tokens || '?'} tokens)`);
      return content;
    } finally {
  ...
  private heuristicAnalysis(market: MarketData, risk: RiskSnapshot): AIAnalysis {
    ...
    if (threats.length === 0) {
      ...
      insights.push(`XLM stable at $${market.price.toFixed(4)} with ${market.priceChange24h > 0 ? '+' : ''}${market.priceChange24h.toFixed(2)}% 24h change`);
    } else {
    ...
    insights.push(`Liquidity: $${(market.liquidity / 1e6).toFixed(1)}M (${market.liquidityChange > 0 ? '+' : ''}${market.liquidityChange.toFixed(1)}%)`);
    insights.push(`Risk engine composite: ${risk.overallRisk}/100`);

    const reasoning = `XLM trading at $${market.price.toFixed(4)} with ${market.priceChange24h > 0 ? '+' : ''}${market.priceChange24h.toFixed(2)}% 24h movement. ` +
      `Volume at $${(market.volume24h / 1e6).toFixed(0)}M (${market.volumeChange > 0 ? '+' : ''}${market.volumeChange.toFixed(0)}% change). ` +
      `Stellar ecosystem liquidity at $${(market.liquidity / 1e6).toFixed(1)}M. ` +
  ...
  private heuristicThreatReport(threat: ThreatAssessment, market: MarketData): string {
    if (!threat.threatDetected) {
      return `All Clear: XLM at $${market.price.toFixed(4)} with no significant threats. All risk vectors within normal parameters. Continuing automated monitoring.`;
    }
    ...
    return `${severityLabels[threat.severity]} ALERT: ${threat.threatType} detected with ${threat.confidence}% confidence. ` +
      `XLM at $${market.price.toFixed(4)} (${market.priceChange24h > 0 ? '+' : ''}${market.priceChange24h.toFixed(2)}%). ` +
